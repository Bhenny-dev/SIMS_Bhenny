...
import { getLeaderboard, getUsers, STORAGE_KEYS, requestToJoinTeam, cancelJoinRequest, updateTeam, getEvents, updateTeamRoster, manageJoinRequest, getTeamUsers } from '../services/api.ts';
...
export const TeamHub: React.FC<TeamHubProps> = ({ team, allUsers, allEvents, onClose }) => {
  ...
  const handleSaveLeadership = async () => {
    ...
    const handleManageRequest = async (userId: string, action: 'accept' | 'reject') => {
        if (!team) return;
        try {
            await manageJoinRequest(team.id, userId, action);
            addToast(`Request has been ${action}ed.`, 'success');
        } catch (error: any) {
        ...
        <div className="flex-grow min-h-0 overflow-y-auto p-6">
          ...
          {activeTab === 'leadership' && (
              <div className="space-y-4">
                  ...
                  {!isEditingLeadership ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Card className="p-4"><h5 className="text-sm font-semibold text-slate-500 dark:text-slate-400">Unit Leader</h5><p className="text-lg font-bold text-slate-800 dark:text-slate-100 truncate">{getUserName(team.unitLeader)}</p></Card>
                        <Card className="p-4"><h5 className="text-sm font-semibold text-slate-500 dark:text-slate-400">Unit Adviser</h5><p className="text-lg font-bold text-slate-800 dark:text-slate-100 truncate">{getUserName(team.adviser)}</p></Card>
                        <Card className="p-4"><h5 className="text-sm font-semibold text-slate-500 dark:text-slate-400">Unit Secretary</h5><p className="text-lg font-bold text-slate-800 dark:text-slate-100 truncate">{getUserName(team.unitSecretary)}</p></Card>
                        <Card className="p-4"><h5 className="text-sm font-semibold text-slate-500 dark:text-slate-400">Unit Treasurer</h5><p className="text-lg font-bold text-slate-800 dark:text-slate-100 truncate">{getUserName(team.unitTreasurer)}</p></Card>
                      </div>
                       <Card className="p-4">
                        ...
                        <ul className="mt-2 space-y-1 text-slate-700 dark:text-slate-300 list-disc list-inside">
                          {(team.unitErrands?.filter(e => e) ?? []).length > 0 ? (
                            team.unitErrands?.map((errandId, index) => errandId && <li key={index}>{getUserName(errandId)}</li>)
                          ) : (
          ...
          {activeTab === 'members' && (
              <div className="space-y-6">
                                      ...
                                      return (
                                          <Card key={req.userId} className="p-4 flex justify-between items-center">
                                              ...
                                              <div className="flex gap-2">
                                                  <Button className="text-xs py-1 px-3" onClick={() => handleManageRequest(req.userId, 'accept')}>Accept</Button>
                                                  <Button variant="danger" className="text-xs py-1 px-3" onClick={() => handleManageRequest(req.userId, 'reject')}>Reject</Button>
                                              </div>
                                          </Card>
                                      )
                                  })}
                  ...
                  <div>
                      <h3 className="font-bold text-lg mt-4">Team Members & Prospects</h3>
                      <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-4 text-sm text-indigo-700 dark:text-indigo-300 mb-4">
...
const Teams: React.FC = () => {
  ...
  const handleCancelRequest = async () => {
    if (!user) return;
    try {
        await cancelJoinRequest(user.id);
  ...
  const userHasPendingRequest = useMemo(() => {
      if (!user) return false;
      return allTeams.some(team => team.joinRequests?.some(req => req.userId === user.id));
  }, [allTeams, user]);
