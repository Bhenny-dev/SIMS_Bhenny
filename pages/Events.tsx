...
const EventDetailsModal: React.FC<{ event: Event | null; isOpen: boolean; onClose: () => void }> = ({ event, isOpen, onClose }) => {
...
const EventCard: React.FC<{ event: Event; onSelect: (action: 'view' | 'edit' | 'delete', event: Event) => void; canManage: boolean }> = ({ event, onSelect, canManage }) => {
  ...
  return (
    <Card className={`flex flex-col transform transition-all hover:-translate-y-1 border ${cardStyles[status]} dark:shadow-soft-dark overflow-hidden`}>
      <div className="p-5 flex-grow">
        <div className="flex justify-between items-start mb-3">
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 leading-tight">{event.name}</h3>
          <span className={`px-2.5 py-1 text-xs font-semibold rounded-full uppercase tracking-wide ${badgeStyles[status]}`}>
            {status}
          </span>
        </div>
        <div className="space-y-2.5 mb-4 text-sm opacity-90 text-slate-600 dark:text-slate-300">
          <p className="flex items-center gap-2">
            <i className="bi bi-person-badge-fill text-slate-400"></i>
            <span className="truncate">{event.officerInCharge}</span>
          </p>
          <p className="flex items-center gap-2">
            <i className="bi bi-calendar-week text-slate-400"></i>
      ...
      <div className="px-4 py-3 bg-white/50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700 flex gap-2">
        <Button variant="secondary" className="flex-1 text-xs py-1.5" onClick={() => onSelect('view', event)}>View</Button>
        {canManage && (
            <>
                <Button variant="secondary" className="px-2 py-1.5 text-xs" onClick={() => onSelect('edit', event)}><i className="bi bi-pencil-fill"></i></Button>
                <Button variant="danger" className="px-2 py-1.5 text-xs" onClick={() => onSelect('delete', event)}><i className="bi bi-trash-fill"></i></Button>
            </>
        )}
      </div>
    </Card>
...
const EventForm: React.FC<{ event: Partial<Event> | null; onSave: (event: Partial<Event>) => void; onClose: () => void }> = ({ event, onSave, onClose }) => {
    ...
    const inputClass = "mt-1 block w-full text-sm px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-all duration-150 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none";
...
const DeleteConfirmationModal: React.FC<{ event: Event | null; isOpen: boolean; onClose: () => void; onConfirm: () => void; }> = ({ event, isOpen, onClose, onConfirm }) => {
    if (!event) return null;
    return (
...
const Events: React.FC = () => {
  ...
  const [modalState, setModalState] = useState<{ type: 'form' | 'details' | 'delete' | null; event: Event | Partial<Event> | null }>({ type: null, event: null });
  const { user } = useAuth();
  const { addToast } = useToast();
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      setEvents(data);
      // Check for eventId param
      const eventIdParam = searchParams.get('eventId');
      if (eventIdParam) {
          const foundEvent = data.find(e => e.id === eventIdParam);
          if (foundEvent) {
              setModalState({ type: 'details', event: foundEvent });
          }
      }
    } catch (error) {
  ...
  const handleSelect = (action: 'view' | 'edit' | 'delete', event: Event) => {
      if (action === 'view') setModalState({ type: 'details', event });
      if (action === 'edit') setModalState({ type: 'form', event });
      if (action === 'delete') setModalState({ type: 'delete', event });
  }

  const handleOpenAddForm = () => {
      setModalState({ type: 'form', event: null });
  }
  ...
  const handleSaveEvent = async (eventData: Partial<Event>) => {
      try {
          ...
          handleCloseModals();
          fetchEvents(); // Refetch data after save
      } catch (error) {
  ...
  const handleDeleteEvent = async () => {
      if (modalState.event && 'id' in modalState.event) {
          try {
              await apiDeleteEvent(modalState.event.id as string);
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={modalState.type === 'form'} onClose={handleCloseModals} title={modalState.event?.id ? `Edit: ${modalState.event.name}` : "Add New Event"}>
          <EventForm event={modalState.event} onSave={handleSaveEvent} onClose={handleCloseModals} />
      </Modal>

      <EventDetailsModal event={modalState.type === 'details' ? modalState.event as Event : null} isOpen={modalState.type === 'details'} onClose={handleCloseModals} />
      
      <DeleteConfirmationModal event={modalState.type === 'delete' ? modalState.event as Event : null} isOpen={modalState.type === 'delete'} onClose={handleCloseModals} onConfirm={handleDeleteEvent} />
    </div>
  );
};
